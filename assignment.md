...
[1단계] preprocess.py 직접 작성
    → report_final.csv 정제 (휴장일 제거 + 장중 공시만 필터링)
         ↓
...
[한국투자증권 KIS Developers](https://apiportal.koreainvestment.com/)에서 회원가입 후 앱을 등록하고
**APP KEY / APP SECRET**을 발급받습니다. (실전투자 계좌 기준)
프로젝트 루트에 `.env` 파일을 생성하고 아래 형식으로 입력합니다.
...
프로젝트 루트에 `data` 폴더가 있고, 그 안에 이전 과제 결과물인 `report_final.csv`가 존재하는지 확인합니다.
...
> **왜 이 전처리가 필요한가?**
> 기준 시점 t 에서 분봉을 세려면 t 가 **실제로 거래가 일어나는 시각**이어야 합니다.
>
> - 휴장일(주말·공휴일) 공시는 그날 분봉 자체가 없습니다.
> - 장 마감(15:30) 직전·이후 공시는 t 시점에 대응하는 분봉이 없어 봉 개수가 어긋납니다. 마감 직전 여유를 두기 위해 **15:20**까지로 자릅니다.
...
두 마스크를 모두 만족하는 행만 남겨(`df[_is_trading_day & _in_session]`) `data/report_final.csv`를 덮어씁니다.
...
모든 행의 결과를 합쳐 아래 컬럼 순서의 DataFrame으로 만들고 `data/price.csv`로 저장합니다.
...
`data/price.csv`가 생성되고, 공시 1건당 1001개의 분봉이 담겨 있으면 성공입니다. 한 종목의 봉이 여러 거래일에 걸쳐 끊김 없이 이어지는지 확인합니다.
...
- [ ] `data/report_final.csv`(이전 과제 결과물) 존재 확인
